...
import {
  ...
  findChangedIndex,
  fixLeadingZero,
  getCharacterNormalizeInternational,
  getCurrentCaretPosition,
  isNil,
  isNormalizeInternationalNumber,
  limitToScale,
  noop,
  normalizeNumberOrWord,
  omit,
  returnTrue,
  roundToPrecision,
  setCaretPosition,
...
class NumberFormat extends React.Component<any> {
  state: {
    value?: string;
    numAsString?: string;
    mounted: boolean;
  ...
  /** Misc methods **/
  getFloatString(num = '') {
    const { localize, decimalScale } = this.props;
  ...
  getNumberRegex(g: boolean, ignoreDecimalSeparator?: boolean) {
    const { localize, format, decimalScale } = this.props;
  ...
  removeFormatting(val: string) {
    const { localize, format, removeFormatting } = this.props;
    if (!val) return val;

    if (!format) {
      val = this.removePrefixAndSuffix(val);
      val = localize ? this.getFloatString(val, localize) : this.getFloatString(val);
  ...
  updateValue(
    ...
    if (formattedValue !== lastValue) {
      // Make sure the number that user has been input is international number
      if (isNormalizeInternationalNumber(formattedValue) && localize) {
  ...
  onChange(e: any) {
    ...
    const { localize, isAllowed } = props;
    ...
    let numAsString = localize
      ? this.removeFormatting(formattedValue, localize)
      : this.removeFormatting(formattedValue);

    if (localize) {
      numAsString = normalizeNumberOrWord(numAsString);
    }
    ...
    if (localize) {
      this.updateValue({ formattedValue, numAsString, inputValue, input: el }, localize);
    } else {
      this.updateValue({ formattedValue, numAsString, inputValue, input: el });
    }

    if (isChangeAllowed) {
      props.onChange(e, numAsString);
    }
  }
  ...
  render() {
    const { type, displayType, customInput, renderText, getInputRef, format, inputProps: otherInputProps } = this.props;
