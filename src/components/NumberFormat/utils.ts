...
export function japaneseRegExp() {
  return /[-[\]/{}()*+?.\\^$|]+|[0-9]+|[０-９]+/u;
}

export const numberNormalizeType = new RegExp('[０-９]');
export const numberNormalizeNum = /[０-９]/g;
...
export function normalizeNumberOrWord(value: string) {
  return value.toString().toLocaleLowerCase().normalize();
}

export function compareNormalize(value: string, compareValue: string, format: any) {
  return value.toString().toLocaleLowerCase() === compareValue;
}

export function isNormalizeInternationalNumber(value: string) {
  return numberNormalizeType.test(value);
}

export function getCharacterNormalizeInternational(value: string) {
  return value.match(numberNormalizeNum);
}
